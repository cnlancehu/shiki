...
`shiki-core` is the core crate of `shiki-rs`. It compiles TextMate grammars into a
compact runtime representation, tokenizes with Oniguruma, resolves themes, and
provides HTML and custom rendering APIs.

The Rust library is named `shiki_core`. Its published Cargo package is
`shiki-rs-core`, because the `shiki-core` package name on crates.io belongs to
...
```console
cargo add shiki-rs-core
```

For the usual bundled setup:

```console
cargo add shiki-rs-core shiki-langs shiki-themes
```
...
```rust,ignore
let mut highlighter = shiki_core::Highlighter::builder()
    .languages(["text"])
...
```rust,ignore
let mut options = shiki_core::HtmlOptions::default();
...
static HTML: LazyLock<shiki_core::HtmlOptions> = LazyLock::new(|| {
    let mut options = shiki_core::HtmlOptions::clean();
    options.pre_classes.push("code-block".into());
    options
        .code_attributes
...
```console
cargo add shiki-rs-core --no-default-features
```

In that configuration, target-side `shiki-core` depends on `onig_sys`, `thiserror`,
